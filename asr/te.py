...
if __name__ == '__main__':
    ...
    if args.continue_from:  # Starting from previous model
        ...
        with open("star_labels.json") as label_file:
            slabels = str(''.join(json.load(label_file)))
        
        model_e = DeepSpeech(rnn_hidden_size=args.hidden_size,
                           nb_layers=args.hidden_layers,
                           labels=slabels,
                           rnn_type=supported_rnns[rnn_type],
                           audio_conf=audio_conf,
    ...
    e_train_dataset = SpectrogramDataset(audio_conf=audio_conf, manifest_filepath='/home/hemant/asr_wm/data/ner/pp_without_misc/star_without_space/train.csv',
                                         labels=slabels, normalize=True, augment=args.augment)
    ...
    e_train_loader = AudioDataLoader(e_train_dataset,
                                   num_workers=args.num_workers, batch_sampler=train_sampler)
    ...
    try: 
        ...
        model_e.load_state_dict(torch.load('models/star_dummy.pth')['state_dict'],strict=False)
        print('loading weights')
    except: pass
    model = model.to(device)
    model_e = model_e.to(device)
    parameters = model.parameters()
    e_parameters = model_e.parameters()
    optimizer = torch.optim.SGD(parameters, lr=args.lr,
                                momentum=args.momentum, nesterov=True, weight_decay=1e-5)
    e_optimizer = torch.optim.SGD(e_parameters, lr=args.lr,
                                momentum=args.momentum, nesterov=True, weight_decay=1e-5)
    ...
    model_e, e_optimizer = amp.initialize(model_e, e_optimizer,
                                      opt_level=args.opt_level,
                                      keep_batchnorm_fp32=args.keep_batchnorm_fp32,
                                      loss_scale=args.loss_scale)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            if i == len(train_sampler):
                break
            e_inputs, e_targets, e_input_percentages, e_target_sizes = iter(e_train_loader)
            inputs, targets, input_percentages, target_sizes = data
            input_sizes = input_percentages.mul_(int(inputs.size(3))).int()
            e_input_sizes = e_input_percentages.mul_(int(e_inputs.size(3))).int()
            ...
            e_out, e_output_sizes = model(e_inputs, e_input_sizes)
            e_out = e_out.transpose(0, 1)
